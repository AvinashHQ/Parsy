...
|---|---:|---|---|
| Digital PDF | M2 intake/routing complete; L3 target in M2.5 | Docling/native parse + Qwen mapping candidate | Real holdout gate required for support claim |
| Scanned PDF | M2 intake/routing complete; L3 target in M2.5 | PaddleOCR-VL + Qwen mapping candidate | Quality warnings required |
| JPEG/PNG | M2 intake/routing complete; L3 target in M2.5 | PaddleOCR-VL + Qwen mapping candidate | Orientation and resolution checks |
| TIFF | M2 intake/routing complete; L3 target in M2.5 | Normalize pages, PaddleOCR-VL + Qwen candidate | Bounded page count |
...
## Functional fixture coverage

The post-M2.5 package includes ingest/routing and model-benchmark fixtures for every first-live visual format, a UBL-shaped XML route, a hybrid PDF with embedded XML, unknown structured quarantine, and safe-rejection inputs. See `samples/synthetic_corpus/manifest.csv`.
