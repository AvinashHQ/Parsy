...
- Provider-neutral local inference service behind an adapter.
- M2 provider-neutral extraction contract, already completed and frozen.
- M2.5 candidate implementations: Qwen3-VL-4B-Instruct, PaddleOCR-VL-1.6 full pipeline, and Docling/native parsing.
- Deterministic structured parsers and versioned region packs.
- External model APIs disabled by default.
...
| Local extraction service | M2.5 implementation behind the frozen M2 provider contract; candidate route uses Docling/PaddleOCR-VL plus Qwen3-VL |
...
2. After M2.5 is enabled, prefer Docling/native PDF parsing for reliable digital text and PaddleOCR-VL for scans, layout, tables, and evidence.
3. Send normalized pages or parsed page content to Qwen3-VL through the frozen M2 provider contract.
4. Reject schema-invalid output.
5. Run one targeted repair only for explicitly repairable fields.
6. Run universal validation.
7. Resolve and optionally run a benchmarked region pack.
8. Store immutable candidate revision and findings.
...
- `extract_visual`: local parser/VLM calls; low initial concurrency.
...
## Post-M2 implementation status

M0-M2 are recorded as complete and frozen based on project-owner status. Open-source parser/model integration is M2.5. The next critical-path boundary is M2.5, while M3 may begin fixture-driven UI work in parallel. Rails controllers must consume candidate revisions and findings; they must not invoke models directly. See `docs/18_M2_FROZEN_AND_M2_5_HANDOFF.md`.

## Local model-service boundary

The local inference service is a separate Python process or host with an HTTP contract. It may use MLX-VLM during Apple Silicon development and a different compatible runtime in production. The Rails domain contract is invariant across runtimes. Every processing attempt records the model repository, exact revision, quantization, runtime version, prompt hash, parser/OCR versions, device class, latency, and memory/cost metadata.
