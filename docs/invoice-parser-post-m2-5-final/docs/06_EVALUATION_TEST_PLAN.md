...
## Post-M2.5 fixture layers

The evaluation system now has two explicitly separate corpora:

1. `samples/synthetic_corpus/` - 29 deterministic functional and negative fixtures for routing, schema, validation, review, export, and quarantine regression.
2. Permissioned real-document corpus - required for model selection, prompt tuning, and launch accuracy gates.

Synthetic fixtures must never be reported as customer accuracy evidence. Use `docs/20_SYNTHETIC_CORPUS_GUIDE.md` and `docs/22_MODEL_BENCHMARK_RUNBOOK.md`. Open-source model selection and accuracy evaluation are M2.5 activities; M2 remains frozen.
